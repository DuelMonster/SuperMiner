...
public enum EnumFacing implements IStringSerializable
{
    ...
    /**
     * Get a Facing by it's index (0-5). The order is D-U-E-W-N-S. Named getFront for legacy reasons.
     */
    public static EnumFacing getFromSideHit(int index)
    {
        if (index == 1) return EnumFacing.UP;
        else if (index == 2) return EnumFacing.EAST;
        else if (index == 3) return EnumFacing.WEST;
        else if (index == 4) return EnumFacing.NORTH;
        else if (index == 5) return EnumFacing.SOUTH;
