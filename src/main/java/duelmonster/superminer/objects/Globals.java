...
public class Globals {
	...
	public static void stackItems(World world, EntityPlayer entityplayer, SMPacket p) {
		stackItems(world, entityplayer, AxisAlignedBB.fromBounds(p.oPos.getX(), p.oPos.getY(), p.oPos.getZ(), p.oPos.getX() + 1.0D, p.oPos.getY() + 1.0D, p.oPos.getZ() + 1.0D));
	}
	...
	public static void playSound(World world, SoundType sound, BlockPos oPos) {
		world.playSoundEffect((double)((float)oPos.getX() + 0.5F), (double)((float)oPos.getY() + 0.5F), (double)((float)oPos.getZ() + 0.5F), sound.getStepSound(), (sound.getVolume() + 1.0F) / 2.0F, sound.getFrequency() * 0.8F);
	}
