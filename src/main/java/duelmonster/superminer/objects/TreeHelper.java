...
public class TreeHelper {
		...
		for (int yLeaf = currentPacket.oPos.getY() - 1; yLeaf < oPlayer.worldObj.getHeight(); yLeaf++)
			for (int xLeaf = -1; xLeaf < 1; xLeaf++)
				for (int zLeaf = -1; zLeaf < 1; zLeaf++) {
					...
					if (nextBlock.getMaterial(nextState) == Material.leaves && Globals.isIdInList(nextBlock, myGlobals.lLeafIDs)) {
	...
	public String getUniqueIdentifier(IBlockState state, int iMetadata) {
		...
		boolean bIsValidWood = (block.getMaterial(state) == Material.wood && Globals.isIdInList(block, myGlobals.lBlockIDs));
		boolean bIsValidLeaves = (block.getMaterial(state) == Material.leaves && Globals.isIdInList(block, myGlobals.lLeafIDs));
	...
	public boolean processPosition(SMPacket currentPacket, BlockPos nextPos, boolean bIsTree) {
		...
		if (nextBlock != null && nextBlock != Blocks.air && ensureIsConnectedToTree(currentPacket, nextPos)) {
			boolean bIsValidWood = (nextBlock.getMaterial(nextState) == Material.wood && Globals.isIdInList(nextBlock, myGlobals.lBlockIDs));
			boolean bIsValidLeaves = (nextBlock.getMaterial(nextState) == Material.leaves && Globals.isIdInList(nextBlock, myGlobals.lLeafIDs));
