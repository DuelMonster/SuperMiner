...
public class Lumbinator {
	...
	public void onServerPacket(FMLNetworkEvent.ServerCustomPacketEvent event) {
		...
		if (iPacketID == PacketIDs.Settings_Lumbinator.value()) {
			...
			if (oPlayer == null) oPlayer = ((NetHandlerPlayServer)event.handler).playerEntity;
	...
	private static boolean isChoppableTree(SMPacket currentPacket) {
		...
		if (block == null || block.getMaterial() != Material.wood || !Globals.isIdInList(block, myGlobals.lBlockIDs)) return false;
		...
		if (!currentPacket.positions.contains(currentPacket.oPos)) currentPacket.positions.offer(currentPacket.oPos);
	...
	private static void chopTree(SMPacket currentPacket) {
		...
		while (iCount < BREAK_LIMIT && !currentPacket.positions.isEmpty())
			if (breakBlock(currentPacket)) iCount++;
		...
		if (currentPacket.positions.isEmpty()) {
			...
			Globals.stackItems(oPlayer.worldObj, oPlayer, AxisAlignedBB.getBoundingBox(currentPacket.oPos.getX(), currentPacket.oPos.getZ(), currentPacket.oPos.getZ(), currentPacket.oPos.getX(), currentPacket.oPos.getZ(), currentPacket.oPos.getZ()).expand(8, 8, 8));
