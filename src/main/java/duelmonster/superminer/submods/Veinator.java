...
public class Veinator {
	...
	public static Boolean isExcavating() {
		...
		for (ExcavationHelper oEH : getMyExcavationHelpers())
			if (!bIsExcavating) bIsExcavating = (oEH != null && oEH.isExcavating());
	...
	public static void syncConfig() {
		...
		if (!bOresGot) {
			bOresGot = true;
			// Check the Forge OreDictionary for any extra Ores not included in
			// the config file.
			String[] saOreNames = OreDictionary.getOreNames();
			for (String sOreName : saOreNames)
				if (sOreName.startsWith("ore")) for (ItemStack item : OreDictionary.getOres(sOreName))
	...
	public void tickEvent(TickEvent.ClientTickEvent event) {
		if (!PlayerEvents.IsPlayerInWorld() || Excavator.isToggled() || Shaftanator.bToggled || !SettingsVeinator.bEnabled || !TickEvent.Phase.END.equals(event.phase)) return;
		...
		if (world != null) {
			...
			if (player.getHealth() > 0.0F && mc.objectMouseOver != null && mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK) {
				...
				if (!Globals.isAttacking(mc) && block != null && block == Blocks.AIR) block = null;
			}
			...
			if (block != null && Globals.isIdInList(block, myGlobals.lBlockIDs)) {
				...
				if (player.getFoodStats().getFoodLevel() > Globals.MIN_HUNGER) myGlobals.addAttackBlock(player, state, oPos, false, true, true, false);
			...
			for (Iterator<SMPacket> attackPackets = myGlobals.attackHistory.iterator(); attackPackets.hasNext();) {
				...
				if (System.nanoTime() - packet.nanoTime >= Globals.attackHistoryDelayNanoTime) attackPackets.remove(); // Removes
																														// packet
																														// from
																														// the
																														// history
					...
					if (block == null || block == Blocks.AIR) {
						attackPackets.remove(); // Removes packet from the
												// history.
	...
	public void tickEvent_Server(TickEvent.ServerTickEvent event) {
		...
		// Retrieve any FMLInterModComm messages that may have been sent from
		// other mods
		...
		if (myExcavationHelpers.size() > 0) for (ExcavationHelper oEH : getMyExcavationHelpers())
	...
	private static boolean isAllowedToMine(EntityPlayer player, SMPacket p) {
		...
		if (state.getMaterial().isToolNotRequired() || oEquippedItem == null || oEquippedItem.stackSize <= 0 || !Globals.isIdInList(oEquippedItem.getItem(), myGlobals.lToolIDs) || !oEquippedItem.canHarvestBlock(state) || !ForgeHooks.canToolHarvestBlock(player.world, p.oPos, oEquippedItem)) return false;
	...
	public void processIMC(final FMLInterModComms.IMCMessage imcMessage) {
		if (imcMessage.key.equalsIgnoreCase("MineVein")) if (imcMessage.isNBTMessage()) {
